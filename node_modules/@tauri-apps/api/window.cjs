...
class Window {
    ...
    async listen(event$1, handler) {
        ...
        return event.listen(event$1, handler, { target: this.label });
    }
    /**
    ...
    async once(event$1, handler) {
        ...
        return event.once(event$1, handler, { target: this.label });
    }
    /**
    ...
    async emit(event$1, payload) {
        ...
        return event.emit(event$1, payload, { target: this.label });
    }
    /** @ignore */
    ...
    async onFileDropEvent(handler) {
        const unlistenFileDrop = await this.listen(event.TauriEvent.WINDOW_FILE_DROP, (event) => {
            handler({
                ...event,
                payload: {
                    type: 'drop',
        ...
        const unlistenFileHover = await this.listen(event.TauriEvent.WINDOW_FILE_DROP_HOVER, (event) => {
            handler({
                ...event,
                payload: {
                    type: 'hover',
        ...
        const unlistenCancel = await this.listen(event.TauriEvent.WINDOW_FILE_DROP_CANCELLED, (event) => {
